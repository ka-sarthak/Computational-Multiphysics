...
def mech_equi(phi,lambda0,C0,C1,Ch,E0,E1,Ebar,tol=1e-5,max_iter=1000,verbose=False):
	...
	delta_up_ft_old = np.zeros(nx,dtype=complex)
	...
	while True:
		...
		# if (np.max(np.real(delta_up_ft[1:])-delta_up_ft_old[1:])<tol or iter>=max_iter):
		if (np.max(np.real(delta_up_ft[1:]))<tol or iter>=max_iter):
			break
		delta_up_ft_old = delta_up_ft
		iter+=1
